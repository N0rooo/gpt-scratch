...
# TRYING TO EXPORT TO ONNX FORMAT
def export_to_onnx(model, dummy_input, output_path="bigram_model.onnx"):
    """
    Export the PyTorch model to ONNX format.

    Args:
    - model: The trained PyTorch model
    - dummy_input: A sample input tensor to trace the model's computation
    ...
    try:
        ...
        print("ONNX validation skipped. Install onnx package to verify.")
...
writer = SummaryWriter("runs/shakespeare_model")
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        writer.add_scalar("Loss/validation", losses["val"], iter)
...
open("more.txt", "w").write(generated_text)
